...
func main() {
	...
	go func() {
		...
		msg := Message{"hello hello, iteration is ", 0, false, localIP, id}
		for {
			sendChn <- msg
			msg.Id++
	...
	for {
		...
		case incomming := <-recChn:
			//&& (incomming.LocalIP != localIP)
			if !incomming.Receipt && (incomming.Localid != id) {
				fmt.Printf("Received: %#v\n", incomming.Note, incomming.Id)
				msg := Message{"This is a receipt of message ", incomming.Id, true, localIP, id}
				sendChn <- msg
